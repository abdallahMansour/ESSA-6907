...
function getWarnings(skill, messagesList) {
  ...
  skill.data.assessments.map((assessmentId, assessmentIndex) => {
    ...
    if (skill.data[assessmentId].type === QUIZ_TYPE) {
      ...
      skill.data[assessmentId].questions.map((questionId, quizIndex) => {
        if (
          skill.data[skill.data[assessmentId].questions].questionType === 1 &&
          isEmpty(
            skill.data[skill.data[skill.data[assessmentId].questions].codeId]
              .text,
          )
        ) {
          warnings.data[assessmentIndex + 1] = [
