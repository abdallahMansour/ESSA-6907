...
export function* createHubConnection(action) {
  try {
    const data = yield call(api.createHubConnection, action.onMessageReceivedCallback)
    yield put({
...
export function* sendPatch(action) {
  ...
  try {
    const data = yield call(api.sendLiveShareMessage, action.connection, action.message)
    yield put({
...
export function* sendContentOperation(action) {
  ...
  try {
    const data = yield call(api.publishPageOperations, action.skillId, action.pageId, action.operations)
    yield put({
...
export function* createOrLoadSkillInstanceWatcher() {
  yield takeEvery(CONSTANTS.LOAD_SKILL_INSTANCE_REQUEST, createOrLoadSkillInstance)
}
export function* closeOrUnloadSkillInstanceWatcher() {
  yield takeEvery(CONSTANTS.UNLOAD_SKILL_INSTANCE_REQUEST, closeOrUnloadSkillInstance)
}
...
export function* sendContentOperationWatcher() {
  yield takeEvery(CONSTANTS.SEND_CONTENT_OPERATION_REQUEST, sendContentOperation)
}
...
export function* collaboratorJoinedWatcher() {
  yield takeEvery(CONSTANTS.COLLABORATOR_JOINED_EVENT_REQUEST, collaboratorJoined)
}
...
export function* addContentOperationsWatcher() {
  yield takeEvery(CONSTANTS.ADD_CONTENT_OPERATIONS_REQUEST, addContentOperations)
}
...
export function* liveshareNodeOperationReceivedWatcher() {
  yield takeEvery(CONSTANTS.LIVESHARE_NODE_OPERATION_RECEIVED_REQUEST, liveshareNodeOperationReceived)
}
export function* liveshareLinkOperationReceivedWatcher() {
  yield takeEvery(CONSTANTS.LIVESHARE_LINK_OPERATION_RECEIVED_REQUEST, liveshareLinkOperationReceived)
}


export default function* liveshareSaga() {
  yield all([createHubConnectionWatcher(),
